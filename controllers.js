...
const loadResources = (options) => {
    return new Promise((resolve, reject) => {
        getResources().then((items) => {
            ...
            <ul class="block-list">${items.map(item => `<li class="row"><div class="cell-5"><a href="#/resources/resource:${item.name}?id=${item.id}&&app=${item.kind}">${item.name}</a></div><div class="cell-3">${item.kind}</div><div class="cell-2">${item.lastModified}</div></li>`).join("")}</ul>`
